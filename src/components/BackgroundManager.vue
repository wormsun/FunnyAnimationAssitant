<template>
  <div class="background-manager">
      ...
      <div class="gallery-container">
        ...
        >
          <div 
            ...
            :key="bg.id" 
            class="prop-card"
            :class="{ selected: isBatchMode && selectedIds.has(bg.id) }"
            @click="handleCardClick(bg)"
          >
            <!-- Batch Checkbox -->
            <div
              v-if="isBatchMode"
            ...
            >
              <input 
                type="checkbox" 
                :checked="selectedIds.has(bg.id)"
                @click.stop="toggleSelect(bg.id)"
              >
            </div>
            ...
            <div class="card-info">
              ...
              <div class="prop-meta">
                <div class="tags-row">
                  <span
                    v-for="tag in (bg.tags || []).slice(0, 2)"
                    :key="tag"
                    class="mini-tag"
                  >
                    {{ tag }}
                  ...
                  <span
                    v-if="(bg.tags || []).length > 2"
                    class="more-tags"
                  >...</span>
                </div>
                <span class="time-ago">{{ formatTimeAgo(bg.createdAt || 0) }}</span>
              </div>
            </div>
    ...
    <BackgroundEditorModal 
      ...
      :background-id="editingBackgroundId || ''"
      @close="closeEditor"
      @save="handleSaved"
    />
...
const filteredBackgrounds = computed(() => {
  ...
  if (resourceType.value === 'dynamic') {
    list = list.filter(bg => bg.type === 'animation')
  } else if (resourceType.value === 'static') {
    list = list.filter(bg => bg.type === 'static')
  }

  if (searchKeyword.value) {
    const k = searchKeyword.value.toLowerCase()
    list = list.filter(bg => bg.name.toLowerCase().includes(k))
  }
...
// Methods
function getPreviewUrl(bg: Background): string {
  if (bg.type === 'static') {
    return bg._runtimeUrl ?? getImageUrl(bg.url) ?? ''
  } else {
    // Animation: Prioritize custom still frame, then first frame
    if (bg.stillFrameSource === 'custom') {
      return bg._runtimeStillUrl ?? getImageUrl(bg.stillFrameCustomUrl) ?? ''
    } else {
      // Use frame
      const idx = bg.stillFrameIndex ?? 0
      const frame = bg.frames?.[idx]
      return frame ? (frame._runtimeUrl ?? getImageUrl(frame.url) ?? '') : ''
    }
  }
}
...
function openCreateModal() {
  editingBackgroundId.value = undefined
  editorVisible.value = true
}

function openEditModal(id: string) {
  editingBackgroundId.value = id
  editorVisible.value = true
}
...
function confirmDelete() {
  pendingDeleteIds.value.forEach(id => backgroundStore.deleteBackground(id))
...
function handleCardClick(bg: Background) {
...
function selectAll() {
  filteredBackgrounds.value.forEach(bg => selectedIds.value.add(bg.id))
}
...
function invertSelection() {
  ...
  selectedIds.value.clear()
  filteredBackgrounds.value.forEach(bg => {
    if (!currentSet.has(bg.id)) {
      selectedIds.value.add(bg.id)
    }
  })
}
